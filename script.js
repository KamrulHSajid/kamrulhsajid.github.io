...
(function(){
  ...
  if (reduceMotion) {
    intro.innerHTML = fullText.replace(/_$/, '<span class=\"type-underscore\">_</span>');
    return;
  }
  var text = fullText.endsWith('_') ? fullText.slice(0, -1) : fullText;
  intro.textContent = '';
  ...
  function typeNext(){
    if (i < text.length) {
      ...
      var delay = ch === ' ' ? 62 : (/[,.]/.test(ch) ? 115 : 38);
      setTimeout(typeNext, delay);
    } else {
      intro.classList.remove('typing');
      var cursor = document.createElement('span');
      cursor.className = 'type-underscore';
